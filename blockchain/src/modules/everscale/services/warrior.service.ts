...
export class WarriorService extends BaseContractService {
  ...
  async getHp(): Promise<number> {
    return +(await this.collectionInpageProvider.methods.getHp({}).call())
      .memory;
  }

  async getUnitType(): Promise<string> {
    return (await this.collectionInpageProvider.methods.getUnitType({}).call())
      .memory;
  }

  async getIsHillAvail(): Promise<boolean> {
    return (
      await this.collectionInpageProvider.methods.getIsHillAvail({}).call()
    ).memory;
  }

  async getIsDead(): Promise<boolean> {
    return (await this.collectionInpageProvider.methods.getIsDead({}).call())
      .memory;
  }

  async getAttackPower(): Promise<number> {
    return +(await this.collectionInpageProvider.methods.attackPower({}).call())
      .attackPower;
  }

  async getProtectionPower(): Promise<number> {
    return +(
      await this.collectionInpageProvider.methods.protectionPower({}).call()
    ).protectionPower;
  }

  async getHillingAmount(): Promise<number> {
    return +(await this.collectionInpageProvider.methods.hilling({}).call())
      .hilling;
  }

  async takeAttack(attackerPower: number) {
    return await this.sendTransactionFromWallet(0.02, 'takeAttack', {
      attackerPower,
    });
  }

  async heal(healingAmount: number) {
    return await this.sendTransactionFromWallet(0.02, 'heal', {
      hillingAmount: healingAmount,
    });
  }
}
